...
class UsuariosModel extends Mysql
{
  ...
  public function insertUsuario(string $identificacion, string $nombre, string $apellido, int $telefono, string $direccion, string $email, string $password, int $tipoid, int $status)
  {
    ...
    if (empty($request)) {
      $query_insert = "INSERT INTO persona(identificacion,nombres,apellidos,telefono,direccion,email_user,password,rolid,status) VALUES(?,?,?,?,?,?,?,?,?)";
      $arrData = array(
  ...
  {
    $sql = "SELECT p.idpersona,p.identificacion,p.nombres,p.apellidos,p.telefono,p.direccion,p.email_user,p.status,r.idrol,r.nombrerol 
					  FROM persona p 
					  INNER JOIN rol r
					  ON p.rolid = r.idrol
					  WHERE p.status != 0 ";
